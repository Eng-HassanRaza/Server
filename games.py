...
def grb(link):
		...
		clm = soup.findAll("div",{"class":"details-text"})
                ...
                try:
                        sql = "INSERT INTO BZ_Games VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')"%(tmp1.encode("utf-8","ignore"),tmp2.encode("utf-8","ignore"),tmp3.encode("utf-8","ignore"),tmp6.encode("utf-8","ignore"),tmp4.encode("utf-8","ignore"),tmp5.encode("utf-8","ignore"),tmp7.encode("utf-8","ignore"),tmp8.encode("utf-8","ignore"))
...
def get_gtypes():
        with requests.session() as s:
                x = s.get("https://www.buyzoxs.de/kaufen/spiele-games.html")
...
for gtl in gtypes:
        with requests.session() as e:
                try:
                        ...
                        continue
mycursor.execute("DELETE FROM BZ_Games")
db.commit()
