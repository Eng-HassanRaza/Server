...
def grb(link):
        with requests.session() as a:
			...
			try:
                ...
                try:
                        sql = "INSERT INTO BZ_Music VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')"%(tmp1.encode("utf-8","ignore"),tmp2.encode("utf-8","ignore"),tmp3.encode("utf-8","ignore"),tmp6.encode("utf-8","ignore"),tmp4.encode("utf-8","ignore"),tmp5.encode("utf-8","ignore"),tmp7.encode("utf-8","ignore"),tmp8.encode("utf-8","ignore"))
...
def get_gtypes():
        with requests.session() as s:
                x = s.get("https://www.buyzoxs.de/kaufen/musik-cds.html")
...
for gtl in gtypes:
        with requests.session() as e:
                try:
                        ...
                        continue
mycursor.execute("DELETE FROM BZ_Music")
db.commit()
